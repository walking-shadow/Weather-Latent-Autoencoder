...
class ViT_Decoder(nn.Module):
        ...
        if self.img_size==(721, 1440):
            ...
            for i in range(len(outchans_list)):
                self.final_list.append(
                    nn.ConvTranspose2d(in_channels=dec_dim, out_channels=outchans_list[i],
    ...
    def up_forward(self, x):
        x = self.final_act(self.final_expand(x))  # 在输出之前扩展维度，保证输出的效果更好
    ...
    def forward(self, feat, **kwargs):
        # assert self.img_size==(721, 1440), 'self.img_size should be (721, 1440)'
        B, C, H,W = feat.shape

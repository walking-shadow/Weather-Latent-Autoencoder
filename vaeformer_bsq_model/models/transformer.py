...
from transcoder.models.wave_dynamic_layer import Dynamic_MLP_OFA_Encoder, Dynamic_MLP_OFA_Decoder
...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, 
                attn_mask: Optional[torch.Tensor] = None,
                is_causal: bool =False):
        print(f'x shape in transformer: {x.shape}')
...
class TransformerEncoder(nn.Module):
    def __init__(self,
        ...
        # ZSJ 使用DOFA模型
        self.conv1 = Dynamic_MLP_OFA_Encoder(wv_planes=width, embed_dim=width, kernel_size=self.patch_size[0])
    ...
    def forward(self, x, metadata):
        ...
        x = self.ln_pre(x)
        x = x.permute(1, 0, 2)  # ZSJ ？为什么要把图像的形状从BLC变成LBC，这会导致进入Transformer的时候在B维度上进行自注意力操作
...
class TransformerDecoder(nn.Module):
    def __init__(self,
        ...
        if use_ffn_output:
            self.ffn = nn.Sequential(
                ...
                nn.Tanh(),
            )  # ZSJ 不使用通道倍增，不然会使得conv_out参数量大涨
        #     self.conv_out = nn.Linear(
        #     in_features=dim_ffn_output,
        #     out_features=3 * self.patch_size[0] * self.patch_size[1] * (1 + logit_laplace)
        # )
            self.conv_out = Dynamic_MLP_OFA_Decoder(wv_planes=width, embed_dim=width, kernel_size=self.patch_size[0])
    ...
    def forward(self, x, metadata):
        ...
        x = self.ln_post(x)
        x = self.ffn(x)
        x, weight = self.conv_out(x, metadata)

...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16,patch_stride=16,patch_padding=2, in_chans=3, embed_dim=768):
        ...
        self.proj = nn.Conv2d(embed_dim//8, embed_dim, kernel_size=patch_size, stride=patch_stride, padding=patch_padding,)



    def forward(self, x, metadata, mask=None, **kwargs):
        x = self.proj_dofa(x, metadata)
        x = self.proj(x) 
...
class ViT_Encoder(nn.Module):
    ...
    def __init__(self, 
        ...
        for i in range(0, depth//2):
            ...
            block = Block(
                ...
                window_size=window_size[which_win] if ((i + 1) % interval != 0) else self.patch_embed.patch_shape,  # 选择三种窗口形状，然后选择全图注意力
    ...
    def embedding_forward(self, x, metadata, mask=None, **kwargs):
        x, (self.Hp, self.Wp), mask = self.patch_embed(x, metadata, mask)    
    ...
    def forward(self, input_var, metadata, **kwargs):
        # print(f'input_var x shape:{input_var.shape}')

        x = self.embedding_forward(input_var, metadata, **kwargs)
...
class ViT_Decoder(nn.Module):
    ...
    def decoder_forward(self, x):
        ...
        return x
    def up_forward(self, x, metadata):
        ...
        if self.img_size==(721, 1440):
            ...
            res, weight = self.final_dofa(res, metadata)
    ...
    def forward(self, feat, metadata, **kwargs):
        ...
        out = self.up_forward(out, metadata)

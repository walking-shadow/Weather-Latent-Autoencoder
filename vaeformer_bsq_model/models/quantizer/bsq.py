...
class BinarySphericalQuantizer(nn.Module):
    def __init__(self, embed_dim, beta, gamma0, gamma, zeta,
        ...
        # 由于当embeddin设置的太大的时候，不会使用indexes_to_codes，否则index会超出范围，因此需要用其他方法计算出group_codebook
        # 由于一般都把self.group_size设置为1，所以只针对1的情况来设置self.group_size
        assert self.group_size == 1, 'self.group_size != 1'

        # group_codes = torch.arange(2 ** self.group_size)
        # group_codebook = self.indexes_to_codes(group_codes).float()[:, -group_size:]

        group_codebook = torch.tensor([[-1.], [1.]])

        self.register_buffer('group_codebook', group_codebook, persistent=False)
        print(f'group_codebook in quan: {group_codebook}')
    ...
    def forward(self, z):
        ...
        zq = self.quantize(z)
        zq_compress = ((zq+1)//2).to(dtype=torch.bool)  # 变为0与1二值数据，b,l,c

        # # 不需要index了，直接用latent code就行
        ...
        if self.soft_entropy:
            ...
            # 由于self.soft_entropy是True，所以下面的代码是不执行的，不用担心codebook_entropy函数
            zb_by_sample= ((zq + 1)/2).reshape(z.shape[0], -1, z.shape[-1]).to(torch.float32)    
            persample_entropy = self.get_hard_per_sample_entropy(zb_by_sample)  
            cb_entropy = codebook_entropy(zq, self.basis, self.embed_dim)
            entropy_penalty = self.gamma0 * persample_entropy - self.gamma * cb_entropy

        zq = zq * q_scale  # 实际上就是对binary token 的l2 norm了
        ...
        return (
            ...
            {"H": cb_entropy, "used_codes": used_codes, "indices": indices, "group_indices": group_indices,
             "avg_prob": avg_prob, "zq_compress": zq_compress}
        )

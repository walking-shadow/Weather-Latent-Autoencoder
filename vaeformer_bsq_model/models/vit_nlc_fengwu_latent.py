...
try:
    ...
    ATTENTION_MODE = 'math'
from transcoder.models.wave_dynamic_layer import Dynamic_Conv_OFA_Patch_Embed, Dynamic_MLP_OFA_Projection, Dynamic_Conv_OFA_Encoder, Dynamic_Conv_OFA_Decoder
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16,patch_stride=16,patch_padding=2, in_chans=3, embed_dim=768):
        ...
        # # ZSJ 使用DOFA模型
        # self.proj_dofa = Dynamic_Conv_OFA_Encoder(wv_planes=embed_dim//8, embed_dim=embed_dim//8, kernel_size=1)
...
class ViT_Encoder(nn.Module):
    ...
    def __init__(self, 
        ...
        for i in range(len(inchans_list)):
            self.patch_embed_list.append(
                PatchEmbed(
            img_size=img_size, patch_size=patch_size, patch_stride= patch_stride, patch_padding=patch_padding,
            in_chans=inchans_list[i], embed_dim=enc_dim)
            )
        self.altitude_embedding_list = nn.ModuleList()
        for i in range(len(inchans_list)):
            self.altitude_embedding_list.append(
                nn.Embedding(4, enc_dim)
            )
        self.single_embedding_list = nn.ModuleList()
        for i in range(len(inchans_list)):
            self.single_embedding_list.append(
                nn.Embedding(4, enc_dim)
            )

        self.pe_post_fuse = nn.Linear(len(inchans_list)*enc_dim, embed_dim)
        # self.patch_embed = PatchEmbed(
        #     img_size=img_size, patch_size=patch_size, patch_stride= patch_stride, patch_padding=patch_padding,
        #     in_chans=in_chans, embed_dim=embed_dim)

        num_patches = self.patch_embed_list[0].num_patches

        if use_abs_pos_emb:
            self.pos_embed = nn.Parameter(torch.zeros(1, num_patches, embed_dim), requires_grad=learnable_pos)
            pos_embed = get_2d_sincos_pos_embed(self.pos_embed.shape[-1], self.patch_embed_list[0].patch_shape, cls_token=False)
        ...
        for i in range(0, depth//2):
            ...
            block = Block(
                ...
                window_size=window_size[which_win] if ((i + 1) % interval != 0) else self.patch_embed_list[0].patch_shape,  # 选择三种窗口形状，然后选择全图注意力
    ...
    def embedding_forward(self, x, altitude_embed, single_embed, mask=None, **kwargs):
        data_split = torch.split(x, list(self.inchans_list), dim=-1)
        data_list = []
        for i in range(len(self.inchans_list)):
            # data, (self.Hp, self.Wp), mask = self.patch_embed_list[i](data_split[i])
            data = self.patch_embed_list[i](data_split[i])

            if i != len(self.inchans_list) - 1:
                vari_embed = self.altitude_embedding_list[i](altitude_embed).unsqueeze(1)
                # print(f'data shape:{data.shape}')
                # print(f'vari_embed:{vari_embed.shape}')
                data = data + vari_embed  # 加上各个变量对应的embedding
            else:
                data = data + self.single_embedding_list[i](single_embed).unsqueeze(1)  # 加上各个变量对应的embedding

            data_list.append(data)  # b,l,c
        x = torch.cat(data_list, dim=-1)
        x = self.pe_post_fuse(x)
    ...
    def forward(self, era5_latent_data, altitude_embed, single_embed, **kwargs):
        # era5_latent_data shape: b,l,c
        # print(f'input_var x shape:{input_var.shape}')

        x = self.embedding_forward(era5_latent_data, altitude_embed, single_embed, **kwargs)
...
class ViT_Decoder(nn.Module):
    ...
    def __init__(self, 
        ...
        if self.img_size==(72, 144):
            self.final_expand = nn.Linear(embed_dim, len(outchans_list)*dec_dim)
            self.final_act = nn.GELU()
            self.final_list = nn.ModuleList()
            for i in range(len(outchans_list)):
                self.final_list.append(
                    # nn.Linear(dec_dim, outchans_list[i])
                    nn.Linear(dec_dim, outchans_list[i]*2)  # 进行二通道二分类设置

                )
            # self.final = nn.ConvTranspose2d(in_channels=embed_dim*2, out_channels=len(outchans_list)*dec_dim,
            #                                 kernel_size=patch_size, stride=patch_stride, padding=patch_padding, bias=False)
            # # ZSJ 使用DOFA
            # self.final_dofa = Dynamic_Conv_OFA_Decoder(wv_planes=embed_dim//8, embed_dim=embed_dim//8, kernel_size=1)

        else:
            self.final_expand = nn.Linear(embed_dim, embed_dim*2)
            self.final_act = nn.GELU()
            self.final = nn.ConvTranspose2d(in_channels=embed_dim*2, out_channels=embed_dim//8,
    ...
    def up_forward(self, x):
        x = self.final_act(self.final_expand(x))  # 在输出之前扩展维度，保证输出的效果更好, b,l,c
        # x = x.view(x.size(0), self.Hp, self.Wp,-1)
        # x = x.permute(0, 3, 1, 2)  # b,c,h,w
        if self.img_size==(72, 144):
            split_chans_list = [self.dec_dim]*len(self.outchans_list)
            data_split = torch.split(x, split_chans_list, dim=-1)
            data_list = []
            for i in range(len(self.outchans_list)):
                data = self.final_list[i](data_split[i])
                data_list.append(data)
            x = torch.cat(data_list, dim=-1)

            # res = self.final(x.permute(0, 3, 1, 2))

            # res, weight = self.final_dofa(res)
            # self.last_layer_weight = weight
    ...
    def forward(self, feat, **kwargs):
        # assert self.img_size==(72, 144), 'self.img_size should be (72, 144)'
        B, C, H,W = feat.shape
